...
module.exports = { 
    config: {
        name: "play",
        description: "Play a song/playlist or search for a song from youtube or Soundcloud",
        usage: "",
        category: "music",
        accessableby: "Member",
        aliases: ["p", "pplay"]
        ...
        bot.music.search(args.join(" "), message.author).then(async res => {
            switch (res.loadType) {
                case "TRACK_LOADED":
                    ...
                    message.channel.send(`Added \`${res.tracks[0].title}\` \`${Utils.formatTime(res.tracks[0].duration, true)}\` to the queue!`);
                ...
                case "SEARCH_RESULT":
                    ...
                    collector.on("collect", m => {
                        ...
                        message.channel.send(`Added \`${track.title}\` \`${Utils.formatTime(track.duration, true)}\` to the queue!`);
                        if(!player.playing) player.play();
                    });
                ...
                case "PLAYLIST_LOADED":
                    ...
                    message.channel.send(`Added \`${res.playlist.tracks.length}\` \`${duration}\` tracks in playlist \`${res.playlist.info.name}\` to the queue!`);
                    if(!player.playing) player.play()
                    break;
            }
